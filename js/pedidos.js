...
function obtenerPedidos() {
  try { return JSON.parse(localStorage.getItem(CLAVE_PEDIDOS)) || []; } catch { return []; }
}
function guardarPedidos(pedidos) { localStorage.setItem(CLAVE_PEDIDOS, JSON.stringify(pedidos)); }
function precio(valor) { return new Intl.NumberFormat('es-CL',{style:'currency',currency:'CLP',maximumFractionDigits:0}).format(valor); }
function fecha(valor) { return new Intl.DateTimeFormat('es-CL',{dateStyle:'medium',timeStyle:'short'}).format(new Date(valor)); }
function estadoClase(estado) { return estado.toLowerCase().replaceAll(' ','-'); }
function obtenerSesionActual() { return typeof obtenerSesion === 'function' ? obtenerSesion() : null; }
...
function crearPedidoDesdeCarrito(carrito) {
  ...
  const pedido = {
    ...
    productos: carrito.map(p => ({codigo:p.codigo, nombre:p.nombre, precio:p.precio, cantidad:p.cantidad, unidad:p.unidad, subtotal:p.precio*p.cantidad})),
    total,
    notificaciones: ['Pedido recibido correctamente.','Tu pedido ha sido confirmado y está en preparación.']
  };
...
function renderizarPedidos() {
  ...
  if (contador) contador.textContent = carrito.reduce((s,p)=>s+Number(p.cantidad||0),0);
  if (!lista) return;
...
function imprimirComprobante(id) {
  ...
  ventana.document.write(`<html lang="es"><head><title>Comprobante ${pedido.id}</title><style>body{font-family:Arial,sans-serif;max-width:700px;margin:40px auto;color:#333}h1{color:#2E8B57}table{width:100%;border-collapse:collapse}td,th{padding:10px;border-bottom:1px solid #ddd;text-align:left}.total{font-size:20px;font-weight:bold}</style></head><body><h1>HuertoHogar</h1><h2>Comprobante de pedido</h2><p><b>Número:</b> ${pedido.id}</p><p><b>Fecha:</b> ${fecha(pedido.fecha)}</p><p><b>Cliente:</b> ${pedido.cliente.nombre}</p><table><thead><tr><th>Producto</th><th>Cantidad</th><th>Subtotal</th></tr></thead><tbody>${pedido.productos.map(p=>`<tr><td>${p.nombre}</td><td>${p.cantidad}</td><td>${precio(p.subtotal)}</td></tr>`).join('')}</tbody></table><p class="total">Total: ${precio(pedido.total)}</p><p>Estado: ${pedido.estado}</p><script>window.print()<\/script></body></html>`);
