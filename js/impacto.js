(function(){
  ...
  aporte.textContent = new Intl.NumberFormat('es-CL',{style:'currency',currency:'CLP',maximumFractionDigits:0}).format(valorLocal);
  detalleComunidad.textContent = locales.length ? `${locales.length} línea(s) de productos con origen local identificado.` : 'Aún no hay productos con origen local identificado en tus pedidos.';
